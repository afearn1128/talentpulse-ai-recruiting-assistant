...
/**
 * Coordinates the steps that turn a raw chat transcript into a
 * recruiter-ready brief. Each step.do() call is retried and
 * checkpointed independently by the Workflows engine, so a transient
 * failure in, say, the scoring step doesn't force the summary step
 * to re-run.
 */
  ...
  async run(event: WorkflowEvent<ScreeningWorkflowParams>, step: WorkflowStep): Promise<ScreeningResult> {
    ...
    const recruiterBrief = await step.do("generate-recruiter-brief", async () => {
      const prompt: ChatMessage[] = [
        {
          role: "user",
          content: `Write a short, well-formatted recruiter brief combining this candidate summary and fit score into
a document a recruiter could paste into an ATS note.\n\nCandidate summary: ${candidateSummary}\n\nFit score: ${scoring.score}/10\nRationale: ${scoring.rationale}`,
        },
      ];
